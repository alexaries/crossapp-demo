package loader
{
	...
	public class RPreloader2 extends SparkDownloadProgressBar
	{
		//--------------------------------------------------------------------------
		//
		//  Constructor
		 ...
		 *  @private
		 */
		override public function set backgroundSize(value:String):void
		{
		...
		private function addText():void{
			...
			if(!progressText){
				...
				progressText.defaultTextFormat = txtFormat;
		 ...
		 * 
		 */		
		private function get txtFormat():TextFormat
		{
		...
		{       
			...
			backgroundImage = "assets/n_login/loadProcessBg.jpg";
			if (backgroundImage != null){
				loadBackgroundImage(backgroundImage);
			}
			
			// Determine the size
			var totalWidth:Number = Math.min(stageWidth - 10, 458);
			var totalHeight:Number = 19;
			var startX:Number = Math.round((stageWidth - totalWidth) / 2);
			var startY:Number = Math.round((stageHeight - totalHeight) / 2+106);
			
			addText();
//			progressText.text = "최초 접속시 로딩에 시간이 소요될 수 있습니다. 조금만 기다려주세요...";
			progressText.text = "如果您是第一次来到Project-N，需一点时间加载资源，下次无需加载...";
			progressText.x = startX + 90 + 130;
			progressText.y = startY + 20 + 15;
			...
			_barFrameSprite.x = _barSprite.x = startX - 15 + 133;
			_barFrameSprite.y = _barSprite.y = startY + 15;
		...
		override protected function setDownloadProgress(completed:Number, total:Number):void
		{
			...
			const outerHighlightColors:Array = [0x1847D7, 0x1847D7];
			const outerHighlightAlphas:Array = [0.12, 0.80];
			const fillColors:Array = [0x1847D7, 0x1847D7];
		...
		{
			const highlightColors:Array = [0x1847D7, 0x1847D7];
			const fillColors:Array = [0x1847D7, 0x1847D7];
		...
		private function loadBackgroundImage(classOrString:Object):void
		{
			...
			else if (classOrString && classOrString is String)
			{
				...
				else
				{
					...
					imgLoader.load(new URLRequest(String(classOrString)), loaderContext);      
				}
			}
		}
...
class ErrorField extends Sprite
{
	 ...
	 */ 
	override public function ErrorField(downloadProgressBar:SparkDownloadProgressBar)
	{
		super();
