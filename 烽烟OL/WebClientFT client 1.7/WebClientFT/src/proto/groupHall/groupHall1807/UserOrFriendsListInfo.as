package proto.groupHall.groupHall1807 {
	...
	import proto.groupHall.groupHall1807.UserOrFriendInfo;
	// @@protoc_insertion_point(imports)
	// @@protoc_insertion_point(class_metadata)
	public final class UserOrFriendsListInfo extends Message implements IExternalizable {
		[ArrayElementType("proto.groupHall.groupHall1807.UserOrFriendInfo")]
		public var userOrFriendInfo:Array = [];
		...
		public function writeExternal(output:IDataOutput):void {
			for (var userOrFriendInfoIndex:uint = 0; userOrFriendInfoIndex < userOrFriendInfo.length; ++userOrFriendInfoIndex) {
				WriteUtils.writeTag(output, WireType.LENGTH_DELIMITED, 1);
				WriteUtils.write_TYPE_MESSAGE(output, userOrFriendInfo[userOrFriendInfoIndex]);
			}
			if (hasMaxPage) {
		...
		public function readExternal(input:IDataInput):void {
			...
			while (input.bytesAvailable != 0) {
				...
				switch (tag.number) {
				case 1:
					userOrFriendInfo.push(ReadUtils.read_TYPE_MESSAGE(input, new proto.groupHall.groupHall1807.UserOrFriendInfo));
					break;
				case 2:
					if (maxPageCount != 0) {
