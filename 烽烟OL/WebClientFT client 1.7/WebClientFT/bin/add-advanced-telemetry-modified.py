...
if __name__ == "__main__":
    ...
    infile = sys.argv[1]
    #infile = "E:\\test\\elevator\\products\\game\\bin\\elevator.swf"
    ...
    v = sf.read(1) 
    size=sf.read(4);
    ln = struct.unpack("<I",size )[0]
    print("magic:",magic == b"CWS")
    print("open len:",ln)
    f = zlibfile(sf, magic == b"CWS")

    o = open(infile+".swf", 'wb')
    ...
    while tag != 0:
        ...
        bhead = 2
        longlength = None
        print("code:",code,tag,length,0x3f,length == 0x3f)
        if code==93 or code==92:
            break
