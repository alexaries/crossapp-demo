...
class Register(resource.Resource):
    ...
    """
    def render(self, request):
        username = request.args['username'][0]#角色名
        password = request.args['password'][0]#角色名
        if dbuser.getCharacterIDByUserID(username):
            return '0'
        if dbuser.createUser(username, password):
            return '1'
        else:
            return '0'
